	...
	internal class InventoryManager : HEROsModService
	{
		...
		private static Item MoveItemToContainer(Item _item, byte destination, int invNum)
		{
			...
			if (item.type > 0)
			{
                ...
                {
                    container = Main.chest[player.chest].item;
                }
                else if (destination == 2)
                {
                    container = player.bank.item;
                }
                else if (destination == 3)
                {
                    container = player.bank2.item;
                }
                ...
                for (int i = 0; i < container.Length; i++)  //interate slots in chest
				{
					...
					if (containerItem.type == item.type)  //if chest item is the same type as clicked inventory item
					{
						...
						if (player.chest > -1)
						{
                            ...
                            {
                                NetMessage.SendData(32, -1, -1, null, Main.player[Main.myPlayer].chest, (float)invNum, 0f, 0f, 0);
                            }
                        }
                ...
                {
                    Main.chest[player.chest].item = container;
                }
                else if (destination == 2)
                {
                    player.bank.item = container;
                }
                else if (destination == 3)
                {
                    player.bank2.item = container;
                }
            }
			Recipe.FindRecipes();
			return item;
